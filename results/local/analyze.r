...
r = filter(d, phase == "b", lim_free_mem == 1000, processors == 1)
...
r = d %>% filter(phase == "b", lim_free_mem == 1000) %>% group_by(fs, storage, processors) %>% summarise(obj_per_s = mean(obj_per_s))
...
ggsave("tp-proc-by-storage-1000Memory.png")
